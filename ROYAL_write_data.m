...
switch dataformat
  ...
  case 'snirf'
    ...
    seldat  = startsWith(hdr.chantype, 'nirs', 'IgnoreCase', true); % a royal addition, maybe contains() ?
    ...
    formattedMessage = sprintf('assuming that the nominal wavelengths are %d and %d nm', WL1.nominal, WL2.nominal); % a royal addition
    ft_warning(formattedMessage); % a royal addition
    ...
    % measurementList
    for i=1:size(tra,1)/2 % a royal addition
      source = find(tra(i,:)>0);
    ...
    % stim
    if ~exist('evt') % a royal addition
        evt = []; % a royal addition
    if ~isempty(evt)
