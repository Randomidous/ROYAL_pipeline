function [cfg] = ROYAL_data2bids(cfg, varargin)
...
switch typ
    ...
    case 'raw'
        ...
        % hdr = ft_fetch_header(data); % a royal addition
        if strcmp(cfg.method, 'convert')
...
%%  get optode and header information

opto            = cfg.opto_cfg.opto;
hdr             = cfg.hdr; % a royal addition
...
if need_nirs_json
    ...
    hdr.opto                            = opto; % a royal addition
    ...
    nirs_json = mergestruct(nirs_settings,    nirs_json); % a royal addition
    % nirs_json = mergestruct(generic_settings, nirs_json, false);
    nirs_json = mergestruct(generic_settings, nirs_json); % a royal addition
end
...
% a royal addition end
    ...
    % I do not understand why this line is in here
...
if need_coordsystem_json  % royal comment: this is scuffed and I do not know how to fix it for NIRS
    ...
    coordsystem_json = mergestruct(coordsystem_settings, coordsystem_json); % a royal addition
            ...
            otherwise
                ...
                switch cfg.datatype
                    case {'eeg', 'ieeg', 'emg', 'exg'}
                        ...
                        ft_info('writing ''%s''\n', cfg.outputfile);
                        ROYAL_write_data(cfg.outputfile, dat, 'dataformat', 'brainvision_eeg', 'header', hdr, 'event', trigger);
                    case {'nirs'}
                        % write the data in SNIRF file format
                        [p, f, x] = fileparts(cfg.outputfile);
                        cfg.outputfile = fullfile(p, [f '.snirf']);
                        ft_info('writing ''%s''\n', cfg.outputfile);
                        ROYAL_write_data(cfg.outputfile, dat, 'dataformat', 'snirf', 'header', hdr); % , 'event', trigger); a royal addition
