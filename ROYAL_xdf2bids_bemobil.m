...
if importNIRS % This loop is always executed in current version
    ...
    % try to use information from preprocessing BIDS_config
    % if isfield(BIDS_config.nirs, 'ref_channel')
    %     nirscfg.nirs.nirsReference                 = BIDS_config.nirs.ref_channel; % field name comes from bemobil preprocessing pipeline
    % end
    if isfield(BIDS_config.nirs, 'srate')
        nirscfg.nirs.SamplingFrequency                 = BIDS_config.nirs.srate; % field name comes from bemobil preprocessing pipeline
    end
    if isfield(BIDS_config.nirs, 'linefreq')
        if numel(BIDS_config.nirs.linefreq) == 1
            nirscfg.nirs.PowerLineFrequency           = BIDS_config.nirs.linefreq; % field name comes from bemobil preprocessing pipeline
        elseif numel(BIDS_config.nirs.linefreq) > 1
            nirscfg.nirs.PowerLineFrequency           = BIDS_config.nirs.linefreq(1);
            warning('Only the first value specified in BIDS_config.nirs.linefreq entered in nirs.json')
        end
    end
    ...
    if isfield(nirscfg, 'nirs')
        % [nirscfg.nirs] =  checkfield(nirscfg.nirs, 'nirsReference', 'REF', 'REF');
        [nirscfg.nirs] =  checkfield(nirscfg.nirs, 'PowerLineFrequency', 'n/a', 'n/a');
        [nirscfg.nirs] =  checkfield(nirscfg.nirs, 'SoftwareFilters', 'n/a', 'n/a');
    else
        % nirscfg.nirs.nirsReference = 'REF';
        nirscfg.nirs.PowerLineFrequency = 'n/a';
        nirscfg.nirs.SoftwareFilters = 'n/a';
    end
